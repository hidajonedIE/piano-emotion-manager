...
async function debugRecommendations() {
  ...
  for (const piano of userPianos) {
    ...
    if (lastTuning.length > 0) {
      ...
      const daysSinceService = Math.floor(
        (Date.now() - new Date(lastService.scheduledDate).getTime()) / (1000 * 60 * 60 * 24)
      );
      ...
      console.log(`  Último servicio: ${lastService.scheduledDate}`);
  ...
  allServices.forEach(s => {
    console.log(`  Service ID: ${s.id}, Type: ${s.serviceType}, Piano ID: ${s.pianoId}, Date: ${s.scheduledDate}`);
  });
