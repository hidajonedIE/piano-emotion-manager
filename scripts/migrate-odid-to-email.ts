...
// Mapeo de OpenID a Email
const USER_MAPPING = {
  'user_37Nq41VhiCgFUQldUPyH8fn25j6': 'jnavarrete@inboundemotion.com',
  'test-user-456': 'test2@example.com',
  'test-user-999': 'test@test.com',
};

async function migrateOdIdToEmail() {
  console.log('🔄 Migrando odId de Clerk ID a Email...\n');
      ...
      try {
        ...
        if (records.length > 0) {
          ...
          Object.entries(byOdId).forEach(([odId, count]) => {
            console.log(`   ${odId}: ${count} registros`);
          });
          console.log('');
        }
      } catch (error) {
