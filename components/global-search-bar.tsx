    ...
    clients.forEach((client) => {
      ...
      if (fullName.includes(query) || email.includes(query) || phone.includes(query) || taxId.includes(query)) {
        ...
        results.push({
          ...
          badge: (client as any).clientType === 'particular' ? 'Particular' : 
                 (client as any).clientType === 'school' ? 'Escuela' :
                 (client as any).clientType === 'conservatory' ? 'Conservatorio' :
                 (client as any).clientType === 'store' ? 'Tienda' : 'Empresa',
          icon: 'person.fill',
          color: '#6366F1',
        });
      }
