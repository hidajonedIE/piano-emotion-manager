...
export function ClientMap({ clients, onClientSelect }: ClientMapProps) {
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          const marker = L.marker([lat, lng])
            ...
            .bindPopup(`
              <strong>${client.firstName + ' ' + (client.lastName1 || '')}</strong><br/>
  ...
  const buildAddress = (client: Client): string | null => {
    ...
    if (client.address?.street) {
      ...
      if (client.address?.streetNumber) parts[0] += ` ${client.address?.streetNumber}`;
    }
    if (client.address?.city) parts.push(client.address?.city);
    if (client.address?.province) parts.push(client.address?.province);
    if (client.address?.postalCode) parts.push(client.address?.postalCode);
