...
try {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    const pianoType = pianoTypes[Math.floor(Math.random() * pianoTypes.length)];
    const brand = pianoBrands[Math.floor(Math.random() * pianoBrands.length)];
    const model = `${brand} ${pianoType === 'grand' ? 'C' : 'U'}${Math.floor(Math.random() * 9) + 1}`;
    ...
    const [result] = await connection.execute(
      `INSERT INTO pianos (odId, clientId, brand, model, serialNumber, pianoType, year, partnerId, createdAt, updatedAt) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
      [odId, clientId, brand, model, serialNumber, pianoType, year, partnerId]
    );
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const title = `${serviceType.replace('_', ' ')} - Piano ${pianoId}`;
    const notes = `Cita para ${serviceType.replace('_', ' ')} - Piano ${pianoId}`;
    
    const odIdAppt = `APT${Date.now()}${i}`;
    await connection.execute(
      `INSERT INTO appointments (odId, clientId, pianoId, title, date, serviceType, status, notes, partnerId, createdAt, updatedAt) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
      [odIdAppt, clientId, pianoId, title, date, serviceType, appointmentStatus, notes, partnerId]
    );
  }
  ...
  for (const service of serviceIds) {
    ...
    const [clientData] = await connection.execute('SELECT name, email FROM clients WHERE id = ?', [service.clientId]);
    ...
    await connection.execute(
      `INSERT INTO invoices (odId, invoiceNumber, clientId, clientName, clientEmail, subtotal, taxAmount, total, status, date, dueDate, partnerId, createdAt, updatedAt) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'paid', ?, ?, ?, NOW(), NOW())`,
      [odId, invoiceNumber, service.clientId, clientName, clientEmail, subtotal, taxAmount, service.cost, service.date, dueDate.toISOString().split('T')[0], partnerId]
    );
  }
  ...
  const [appointmentCount] = await connection.execute('SELECT COUNT(*) as total FROM appointments WHERE partnerId = ? AND date >= "2026-02-01" AND date < "2026-03-01"', [partnerId]);
