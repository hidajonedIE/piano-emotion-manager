...
export class ContractService {
  ...
  async createTemplate(
    ...
    const result = await this.db
      ...
      ;
    return result[0] as unknown as ContractTemplate;
  }
  ...
  async getTemplates(organizationId: string): Promise<ContractTemplate[]> {
    return await this.db
      ...
      .orderBy(desc('createdAt' as never)) as unknown as ContractTemplate[];
  }
  ...
  async createContract(
    ...
    const result = await this.db
      ...
      ;
    return result[0] as unknown as MaintenanceContract;
  }
  ...
  async getContracts(
    ...
    return await this.db
      ...
      .orderBy(desc('createdAt' as never)) as unknown as MaintenanceContract[];
  }
  ...
  async getContract(contractId: string): Promise<MaintenanceContract | null> {
    const result = await this.db
      ...
      .limit(1) as unknown as MaintenanceContract[];
  ...
  async recordServiceUsage(
    ...
    // Registrar uso
    await getDb()().insert('contractServiceUsage' as never).values({
      contractId,
      serviceId,
      serviceType,
      usedAt: new Date(),
  ...
  private async createPayment(contract: MaintenanceContract): Promise<void> {
    ...
    await getDb()().insert('contractPayments' as never).values(payment as never);
  ...
  async renewContract(
    ...
    await getDb()().insert('contractRenewals' as never).values({
