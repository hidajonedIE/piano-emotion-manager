...
export class DashboardService {
  ...
  async deleteLayout(layoutId: string): Promise<void> {
    await getDb()().execute(`DELETE FROM dashboard_layouts WHERE id = $1`, [layoutId]);
  }
  ...
  async deleteWidget(widgetId: string): Promise<void> {
    await getDb()().execute(`DELETE FROM dashboard_widgets WHERE id = $1`, [widgetId]);
  }
  ...
  async applyTemplate(layoutId: string, templateId: string): Promise<void> {
    ...
    await getDb()().execute(`DELETE FROM dashboard_widgets WHERE layout_id = $1`, [layoutId]);
  ...
  private async getStatsCardData(organizationId: string, config: WidgetConfig): Promise<StatsCardData> {
    ...
    switch (metric) {
      case 'monthly_revenue':
        const revenueResult = await getDb()().execute(`
          SELECT 
      ...
      case 'services_count':
        const servicesResult = await getDb()().execute(`
          SELECT 
      ...
      case 'active_clients':
        const clientsResult = await getDb()().execute(`
