...
export class ProductService {
  ...
  async create(input: CreateProductInput): Promise<typeof products.$inferSelect> {
    ...
    const existing = await (await getDb())!.query.products.findFirst({
      where: and(
    ...
    const [product] = await (await getDb())!.insert(products).values({
      sku: input.sku,
  ...
  async update(input: UpdateProductInput): Promise<typeof products.$inferSelect> {
    ...
    if (updateData.sku && updateData.sku !== existing.sku) {
      const skuExists = await (await getDb())!.query.products.findFirst({
        where: and(
    ...
    const [updated] = await (await getDb())!.update(products)
      .set({
        ...updateData,
  ...
  async search(
    ...
    const productList = await (await getDb())!.query.products.findMany({
      where: and(...conditions),
      limit: pageSize,
      offset,
    ...
    const stockData = productIds.length > 0 
      ? await (await getDb())!.query.warehouseStock.findMany({
  ...
  async getBrands(organizationId?: number): Promise<string[]> {
    const result = await (await getDb())!.selectDistinct({ brand: products.brand })
      .from(products)
      .where(and(
        eq(products.isActive, true),
        organizationId 
  ...
  async deactivate(id: number): Promise<void> {
    await (await getDb())!.update(products)
      .set({ 
        isActive: false,
        updatedAt: new Date(),
      })
  ...
  async getStatistics(organizationId?: number): Promise<{
    ...
    // Total y activos
    const [totals] = await (await getDb())!.select({
    ...
    // Por tipo
    const byTypeResult = await (await getDb())!.select({
      type: products.type,
    ...
    const stockStats = await (await getDb())!.select({
