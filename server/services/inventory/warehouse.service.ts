...
export class WarehouseService {
  ...
  async create(input: CreateWarehouseInput): Promise<typeof warehouses.$inferSelect> {
    ...
    const existing = await (await getDb())!.query.warehouses.findFirst({
      where: and(
    ...
    if (input.isDefault) {
      await (await getDb())!.update(warehouses)
        .set({ isDefault: false })
    ...
    const [warehouse] = await (await getDb())!.insert(warehouses).values({
      code: input.code,
      name: input.name,
  ...
  async update(input: UpdateWarehouseInput): Promise<typeof warehouses.$inferSelect> {
    ...
    if (updateData.code && updateData.code !== existing.code) {
      const codeExists = await (await getDb())!.query.warehouses.findFirst({
        where: and(
    ...
    const [updated] = await (await getDb())!.update(warehouses)
      .set({
        ...updateData,
  ...
  async getWithStats(id: number): Promise<WarehouseWithStats | null> {
    ...
    const lowStockResult = await (await getDb())!.select({
    ...
    // Productos sin stock
    const outOfStockResult = await (await getDb())!.select({
      count: sql<number>`count(*)`,
    })
      .from(warehouseStock)
      .where(and(
  ...
  async deactivate(id: number): Promise<void> {
    ...
    const [stockCheck] = await (await getDb())!.select({
  ...
  async getAllStats(organizationId?: number): Promise<{
    ...
    const [inventoryStats] = await (await getDb())!.select({
      totalValue: sql<number>`coalesce(sum(${warehouseStock.totalCost}::numeric), 0)`,
      totalProducts: sql<number>`count(distinct ${warehouseStock.productId!})`,
    })
      .from(warehouseStock)
