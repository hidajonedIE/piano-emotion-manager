...
export class BelgiumPeppolService extends BaseEInvoicingService implements IEInvoicingService {
  ...
  async validate(invoice: EInvoice): Promise<{ valid: boolean; errors: string[] }> {
    ...
    if (!invoice.lines || invoice.lines.length === 0) {
      ...
      invoice.lines.forEach((line, index) => {
        ...
        if (line.taxRate /* vatRate */ as any < 0 || line.taxRate /* vatRate */ as any > 100) {
    ...
    if (invoice.taxAmount /* totalAmount */ < 0) {
      errors.push('Le montant total ne peut pas être négatif.');
    }
    
    // Vérification de la cohérence des montants
    const calculatedSubtotal = invoice.lines?.reduce((sum, line) => sum + line.amount /* total */ as any, 0) || 0;
    ...
    if (!invoice.customer.address) {
      ...
      if (!(invoice.customer.address as any).zip) {
        errors.push('Le code postal de l\'adresse du client est obligatoire.');
      }
      if (!(invoice.customer.address as any).countryCode) {
      ...
      _content: [
        ...
        },
        {
          ...
          _content: [
            ...
            { 'cbc:TaxInclusiveAmount': { _attrs: { currencyID: invoice.currency }, _content: invoice.taxAmount /* totalAmount */ } },
            { 'cbc:PayableAmount': { _attrs: { currencyID: invoice.currency }, _content: invoice.taxAmount /* totalAmount */ } },
          ]
        },
        ...invoice.lines.map((line, index) => ({
          ...
          _content: [
            ...
            { 'cbc:LineExtensionAmount': { _attrs: { currencyID: invoice.currency }, _content: line.amount /* total */ as any } },
            {
              _name: 'cac:Item',
              _content: [
                { 'cbc:Name': line.description },
                {
                  ...
                  _content: {
                    ...
                    'cbc:Percent': line.taxRate /* vatRate */ as any,
  ...
  private groupTaxSubtotals(invoice: EInvoice): TaxSubtotal[] {
    ...
    for (const line of invoice.lines || []) {
      const vatRate = line.taxRate /* vatRate */ as any || 0;
      const existing = taxGroups.get(vatRate) || { taxableAmount: 0, taxAmount: 0 };
      existing.taxableAmount += line.amount /* total */ as any;
      existing.taxAmount += line.amount /* total */ as any * (vatRate / 100);
  ...
  private mapAddress(address: AddressInput): UBLAddress {
    return {
      ...
      'cbc:PostalZone': (address as any).zip,
      'cac:Country': { 'cbc:IdentificationCode': (address as any).countryCode },
    };
  }
}
