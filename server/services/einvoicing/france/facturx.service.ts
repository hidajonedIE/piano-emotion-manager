...
export class FacturXService extends BaseEInvoicingService {
  ...
  async generateXML(invoice: EInvoice): Promise<string> {
    ...
    const profile = config?.profile || this.profile;
  ...
  private generateApplicableHeaderTradeSettlement(invoice: EInvoice): string {
    ...
    <ram:ApplicableHeaderTradeSettlement>
      ...
      <ram:SpecifiedTradeSettlementHeaderMonetarySummation>
        ...
        <ram:GrandTotalAmount>${this.formatAmount(invoice.total)}</ram:GrandTotalAmount>
        <ram:DuePayableAmount>${this.formatAmount(invoice.total)}</ram:DuePayableAmount>
      </ram:SpecifiedTradeSettlementHeaderMonetarySummation>
    </ram:ApplicableHeaderTradeSettlement>`;
  }
...
<body>
  ...
  <div class="totals">
    ...
    <p class="total-row">TOTAL TTC: € ${this.formatAmount(invoice.total)}</p>
  </div>
  ...
  protected async validateCountrySpecific(invoice: EInvoice): Promise<string[]> {
    ...
    if (config?.chorusProEnabled && !config?.serviceCode) {
