...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
  ...
  private generateAccountingSupplierParty(invoice: EInvoice): string {
    const config = invoice.countryConfig?.denmark;
    const cvr = config?.taxId /* cvr */ as any || invoice.seller.taxId.replace(/^DK/, '');
  ...
  <cac:LegalMonetaryTotal>
    ...
    <cbc:TaxInclusiveAmount currencyID="${invoice.currency}">${this.formatAmount(invoice.amount /* total */ as any)}</cbc:TaxInclusiveAmount>
    <cbc:PayableAmount currencyID="${invoice.currency}">${this.formatAmount(invoice.amount /* total */ as any)}</cbc:PayableAmount>
  </cac:LegalMonetaryTotal>`;
  }
...
<body>
  ...
  <div class="totals">
    ...
    <p class="total-row">TOTAL INKL. MOMS: ${this.formatCurrency(invoice.amount /* total */ as any, invoice.currency)}</p>
  </div>
  ...
  async send(invoice: EInvoice): Promise<SendResult> {
    try {
      ...
      if (config?.enabled /* nemhandelEnabled */ as any) {
  ...
  private async sendViaPEPPOL(invoice: EInvoice, xml: string, hash: string): Promise<SendResult> {
    ...
    this.log('sendViaPEPPOL', { 
      invoiceNumber: invoice.invoiceNumber,
      peppolId: config?.participantId /* peppolParticipantId */ as any,
      ean: config?.ean,
    });
