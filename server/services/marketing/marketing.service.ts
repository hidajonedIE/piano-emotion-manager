...
export class MarketingService {
  ...
  async getTemplateById(id: number, organizationId: number) {
    const [template] = await (await getDb())!.select()
      .from(messageTemplates)
      .where(and(
  ...
  async createTemplate(data: {
    ...
    if (data.isDefault) {
      await (await getDb())!.update(messageTemplates)
        .set({ isDefault: false })
        .where(and(
    ...
    const [result] = await (await getDb())!.insert(messageTemplates).values({
      ...data,
  ...
  async deleteTemplate(id: number, organizationId: number) {
    await (await getDb())!.delete(messageTemplates)
      .where(and(
        eq(messageTemplates.id, id),
        eq(messageTemplates.organizationId, organizationId)
      ));
  ...
  async getCampaignById(id: number, organizationId: number) {
    const [campaign] = await (await getDb())!.select()
      .from(marketingCampaigns)
      .where(and(
  ...
  async createCampaign(data: {
    ...
    const [result] = await (await getDb())!.insert(marketingCampaigns).values({
      ...data,
      status: 'draft',
    });
  ...
  async updateCampaign(id: number, organizationId: number, data: {
    ...
    await (await getDb())!.update(marketingCampaigns)
      .set(data)
      .where(and(
        eq(marketingCampaigns.id, id),
        eq(marketingCampaigns.organizationId, organizationId)
  ...
  async generateCampaignRecipients(campaignId: number, organizationId: number) {
    ...
    await (await getDb())!.delete(campaignRecipients)
    ...
    for (const client of filteredClients) {
      ...
      await (await getDb())!.insert(campaignRecipients).values({
        campaignId,
        clientId: client.id,
  ...
  async getNextPendingRecipient(campaignId: number) {
    const [recipient] = await (await getDb())!.select({
      recipient: campaignRecipients,
      client: {
        id: clients.id,
        firstName: clients.firstName,
  ...
  async markRecipientAsSent(recipientId: number, sentBy: number) {
    await (await getDb())!.update(campaignRecipients)
      .set({
        status: 'sent',
        sentAt: new Date(),
        sentBy,
  ...
  async logMessage(data: {
    ...
    await (await getDb())!.insert(messageHistory).values({
      ...data,
      channel: 'whatsapp',
      status: 'sent',
    });
