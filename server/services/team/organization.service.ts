...
export class OrganizationService {
  ...
  async create(input: CreateOrganizationInput): Promise<Organization> {
    ...
    const [organization] = await (await getDb())!.insert(organizations).values({
      name: input.name,
      slug,
    ...
    await (await getDb())!.insert(organizationMembers).values({
  ...
  async inviteMember(input: InviteMemberInput): Promise<OrganizationInvitation> {
    ...
    // Crear invitación
    const [invitation] = await (await getDb())!.insert(organizationInvitations).values({
  ...
  async acceptInvitation(token: string, userId: number): Promise<OrganizationMember> {
    ...
    // Crear miembro
    const [member] = await (await getDb())!.insert(organizationMembers).values({
  ...
  private async logActivity(data: {
    ...
    await (await getDb())!.insert(organizationActivityLog).values({
