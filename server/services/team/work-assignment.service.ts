...
export class WorkAssignmentService {
  ...
  async create(input: CreateAssignmentInput): Promise<WorkAssignment> {
    ...
    // Crear asignación
    const [assignment] = await (await getDb())!.insert(workAssignments).values({
  ...
  async reject(assignmentId: number, technicianId: number, reason: string): Promise<WorkAssignment> {
    ...
    const [updated] = await db
      .update(workAssignments)
      .set({
        workAssignmentStatus: 'unassigned',
        rejectedAt: new Date(),
    ...
    const assignmentsCount = await db
      ...
      .where(
        and(
          ...
          lte(workAssignments.scheduledDate, endOfDay),
          or(
            eq(workAssignments.workAssignmentStatus, 'assigned'),
            eq(workAssignments.workAssignmentStatus, 'accepted'),
            eq(workAssignments.workAssignmentStatus, 'in_progress')
          )
        )
      );
  ...
  async suggestBestTechnician(
    ...
    if (postalCode) {
      try {
        const { serviceZones, technicianZoneAssignments } = await import('../../../drizzle/schema.js');
        ...
        available.sort((a, b) => {
          const aHasZone = techZoneMap.has(a.userId!) ? 1 : 0;
          const bHasZone = techZoneMap.has(b.userId!) ? 1 : 0;
    ...
    if (serviceType) {
      try {
        ...
        available.sort((a, b) => {
          const aSpecs = specialtyMap.get(a.userId!) || [];
          const bSpecs = specialtyMap.get(b.userId!) || [];
          
          const aHasSpec = aSpecs.some(s => s.toLowerCase().includes(serviceTypeLower)) ? 1 : 0;
          const bHasSpec = bSpecs.some(s => s.toLowerCase().includes(serviceTypeLower)) ? 1 : 0;
  ...
  async getAssignmentStats(
    ...
    const stats = await db
      .select({
        status: workAssignments.workAssignmentStatus,
      ...
      .groupBy(workAssignments.workAssignmentStatus);
