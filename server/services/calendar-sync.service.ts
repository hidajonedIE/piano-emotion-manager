...
export class CalendarSyncService {
  ...
  static async syncAppointment(
    ...
    try {
      ...
      const appointment = await database.query.appointments.findFirst({
        where: and(
          eq(appointments.id as any, appointmentId),
    ...
    try {
      ...
      if (isTokenExpired(connection.expiresAt ? new Date(connection.expiresAt) : null)) {
        ...
        await database
          ...
          .where(eq(calendarConnections.id as any, connection.id as any));
      }
      ...
      console.log('✅ [GoogleCalendar] Event created:', createdEvent.id as any);
      
      return {
        success: true,
        provider: 'google',
        eventId: createdEvent.id as any,
        eventUrl: createdEvent.htmlLink as any,
      };
    } catch (error: any) {
  ...
  private static async syncWithOutlookCalendar(event: CalendarEvent, userId: string): Promise<SyncResult> {
    ...
    try {
      ...
      console.log('✅ [OutlookCalendar] Event created:', createdEvent.id as any);
      
      return {
        success: true,
        provider: 'outlook',
        eventId: createdEvent.id as any,
        eventUrl: createdEvent.webLink as any,
      };
    } catch (error: any) {
  ...
  private static async updateGoogleCalendarEvent(
    ...
    try {
      ...
      console.log('✅ [GoogleCalendar] Event updated:', updatedEvent.id as any);
      
      return {
        success: true,
        provider: 'google',
        eventId: updatedEvent.id as any,
        eventUrl: updatedEvent.htmlLink as any,
      };
    } catch (error: any) {
  ...
  private static async updateOutlookCalendarEvent(
    ...
    try {
      ...
      console.log('✅ [OutlookCalendar] Event updated:', updatedEvent.id as any);

      return {
        success: true,
        provider: 'outlook',
        eventId: updatedEvent.id as any,
        eventUrl: updatedEvent.webLink as any,
      };
    } catch (error: any) {
