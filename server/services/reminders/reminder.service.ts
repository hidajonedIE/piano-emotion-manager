...
export class ReminderService {
  ...
  async createRule(
    ...
    const result = await (await getDb())!.insert('reminderRules' as never).values(rule as never);
  ...
  async scheduleReminder(params: ScheduleReminderParams): Promise<ScheduledReminder> {
    ...
    const result = await (await getDb())!.insert('scheduledReminders' as never).values(reminder as never);
  ...
  private async logReminderHistory(
    ...
    for (const [channel, result] of Object.entries(results)) {
      ...
      await (await getDb())!.insert('reminderHistory' as never).values({
  ...
  private async findCandidatesForRule(
    ...
    switch (condition.event) {
      case 'last_service': {
        ...
        const pianos = await (await getDb())!.execute(sql`
      ...
      case 'service_completed': {
        ...
        const services = await (await getDb())!.execute(sql`
      ...
      case 'appointment_scheduled': {
        ...
        const appointments = await (await getDb())!.execute(sql`
      ...
      case 'invoice_due': {
        ...
        const invoices = await (await getDb())!.execute(sql`
