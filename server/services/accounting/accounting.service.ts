...
export class AccountingService {
  ...
  async createAccount(input: AccountInput): Promise<typeof financialAccounts.$inferSelect> {
    const [account] = await (await getDb())!.insert(financialAccounts).values({
  ...
  async getAccounts(): Promise<Array<typeof financialAccounts.$inferSelect>> {
    return (await getDb())!.query.financialAccounts.findMany({
      where: and(
  ...
  private async updateAccountBalance(accountId: number, amount: number, isAddition: boolean): Promise<void> {
    const account = await (await getDb())!.query.financialAccounts.findFirst({
  ...
  async createTransaction(input: TransactionInput): Promise<typeof transactions.$inferSelect> {
    ...
    const [transaction] = await (await getDb())!.insert(transactions).values({
      organizationId: this.organizationId!,
      userId: this.userId!,
  ...
  async getTransactions(
    ...
    const [result, countResult] = await Promise.all([
      (await getDb())!.query.transactions.findMany({
  ...
  async deleteTransaction(transactionId: number): Promise<void> {
    const transaction = await (await getDb())!.query.transactions.findFirst({
      where: and(
    ...
    await (await getDb())!.delete(transactions).where(eq(transactions.id, transactionId));
  }
  ...
  async getFinancialSummary(startDate: string, endDate: string): Promise<FinancialSummary> {
    ...
    const allInvoices = await (await getDb())!.query.invoices.findMany({});
  ...
  async getCashFlow(startDate: string, endDate: string): Promise<CashFlowData[]> {
    const txs = await (await getDb())!.query.transactions.findMany({
      where: and(
        eq(transactions.organizationId!, this.organizationId!),
  ...
  async createBudget(
    ...
    const result = await (await getDb())!.insert(budgets).values({
      organizationId: this.organizationId!,
      name,
      startDate,
      endDate,
    ...
    if (lines.length > 0) {
      await (await getDb())!.insert(budgetLines).values(
        lines.map((line) => ({
          budgetId,
    ...
    const [budget] = await (await getDb())!.select().from(budgets).where(eq(budgets.id, budgetId));
    return budget;
  }
  ...
  async getBudgetStatus(budgetId: number): Promise<{
    ...
    const expenses = await (await getDb())!.query.transactions.findMany({
      where: and(
        eq(transactions.organizationId!, this.organizationId!),
  ...
  async getTaxRates(): Promise<Array<typeof taxRates.$inferSelect>> {
    const db = await getDb();
    return (await getDb())!.query.taxRates.findMany({
      where: and(
  ...
  async createTaxRate(
    ...
    const db = await getDb();
    const result = await (await getDb())!.insert(taxRates).values({
      organizationId: this.organizationId!,
      name,
      rate: rate.toString(),
    ...
    const [taxRate] = await (await getDb())!.select().from(taxRates).where(eq(taxRates.id, Number(result.insertId)));
    return taxRate;
  }
}
