...
export class DistributorService {
  ...
  constructor(distributorId: number) {
    (this as any).distributorId = distributorId;
  }
  ...
  async getWooCommerceConfig(): Promise<WooCommerceConfig | null> {
    try {
      ...
      const [config] = await db
        ...
        .where(eq(distributorWooCommerceConfig.distributorId, (this as any).distributorId));
    ...
    try {
      ...
      if (existing) {
        ...
        (await getDb())!.insert(distributorWooCommerceConfig).values({
          ...configData,
          distributorId: (this as any).distributorId,
        });
      }
  ...
  async getPremiumConfig(): Promise<PremiumConfig> {
    try {
      const [config] = await db
        ...
        .where(eq(distributorPremiumConfig.distributorId, (this as any).distributorId));
  ...
  async getTechnicians(): Promise<TechnicianSummary[]> {
    try {
      ...
      const technicians = await db
        ...
        .where(
          eq(technicianAccountStatus.distributorId, (this as any).distributorId)
        )
...
DistributorService.prototype.getModuleConfig = async function(): Promise<ModuleConfig> {
  try {
    ...
    const [config] = await db
      ...
      .where(eq(distributorModuleConfig.distributorId, (this as any).distributorId));
...
DistributorService.prototype.getMyDistributorConfig = async function(): Promise<MyDistributorConfigResult> {
  try {
    ...
    const [accountStatus] = await db
      ...
      .from(technicianAccountStatus)
      .where(eq(technicianAccountStatus.userId!, (this as any).distributorId)); // Note: distributorId here is actually userId
