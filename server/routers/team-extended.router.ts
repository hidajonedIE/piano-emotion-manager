...
async function getMemberByUserId(userId: number, organizationId: number) {
  const [member] = await (await (await getDb())!.getDb())!
    .select()
    ...
    .where(
      and(
        ...
        eq(/* organizationMembers.status */ organizationMembers.id, "active")
      )
    )
    .limit(1);
...
async function getOrganizationAbsences(
  ...
  const absences = await (await (await getDb())!.getDb())!
    .select({
  ...
  return absences.map(({ absence, member }: any) => ({
    ...absence,
...
async function createAbsenceRequest(data: {
  ...
  const [result] = await (await (await getDb())!.getDb())!.insert(memberAbsences).values({
...
async function cancelAbsence(absenceId: number, cancelledBy: number) {
  ...
  const [absence] = await (await (await getDb())!.getDb())!
    .select()
    .from(memberAbsences)
    .where(eq(memberAbsences.id, absenceId))
    .limit(1);
  ...
  await (await (await getDb())!.getDb())!.delete(memberAbsences).where(eq(memberAbsences.id, absenceId));
...
async function getTechnicianMetrics(organizationId: number, technicianId: number, period: string) {
  ...
  const metrics = await (await (await getDb())!.getDb())!
    .select({
...
async function getOrganizationMetrics(organizationId: number, period: string) {
  ...
  const [activeMembers] = await (await (await getDb())!.getDb())!
  ...
  const [pendingAssignments] = await (await (await getDb())!.getDb())!
    .select({ count: sql<number>`COUNT(*)` })
    .from(workAssignments)
    .where(
      and(
        eq(workAssignments.organizationId, organizationId),
        eq(/* workAssignments.status */ workAssignments.id, "assigned")
      )
    );
...
async function getTechnicianRanking(organizationId: number, period: string, metric: string) {
  ...
  const ranking = await (await (await getDb())!.getDb())!
    .select({
...
async function getDashboardStats(organizationId: number) {
  ...
  // Citas de hoy
  const [todayAppointmentsResult] = await (await (await getDb())!.getDb())!
    .select({ count: sql<number>`COUNT(*)` })
    .from(workAssignments)
    .where(
      and(
  ...
  const [pendingResult] = await (await (await getDb())!.getDb())!
    .select({ count: sql<number>`COUNT(*)` })
    .from(workAssignments)
    .where(
      and(
  ...
  // Miembros activos
  const [activeMembersResult] = await (await (await getDb())!.getDb())!
    .select({ count: sql<number>`COUNT(*)` })
    .from(organizationMembers)
    .where(
      and(
  ...
  // Ingresos del mes
  const [monthlyRevenueResult] = await (await (await getDb())!.getDb())!
...
async function getServiceZones(organizationId: number) {
  const zones = await (await (await getDb())!.getDb())!
    .select()
    .from(serviceZones)
  ...
  const zonesWithTechnicians = await Promise.all(
    zones.map(async (zone) => {
      const technicians = await (await (await getDb())!.getDb())!
        .select({
...
async function createServiceZone(data: {
  ...
  const [result] = await (await (await getDb())!.getDb())!.insert(serviceZones).values({
    organizationId: data.organizationId,
    name: data.name,
...
async function deleteServiceZone(zoneId: number) {
  ...
  await (await (await getDb())!.getDb())!.delete(technicianZones).where(eq(technicianZones.zoneId, zoneId));
  
  // Luego eliminar la zona
  await (await (await getDb())!.getDb())!.delete(serviceZones).where(eq(serviceZones.id, zoneId));
...
async function assignTechnicianToZone(zoneId: number, memberId: number, organizationId: number, isPrimary?: boolean) {
  ...
  const [existing] = await (await (await getDb())!.getDb())!
    .select()
    .from(technicianZones)
    .where(
      and(
  ...
  await (await (await getDb())!.getDb())!.insert(technicianZones).values({
    organizationId,
    zoneId,
    memberId,
