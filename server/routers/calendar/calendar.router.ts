...
export const calendarRouter = router({
  ...
  createEvent: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const service = createCalendarService((ctx as any).organizationId, ctx.user.id);
  ...
  getConnections: protectedProcedure.query(async ({ ctx }) => {
    const syncService = createCalendarSyncService(ctx.user.id, (ctx as any).organizationId);
