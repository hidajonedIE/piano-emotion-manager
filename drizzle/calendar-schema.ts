...
export const calendarConnections = pgTable('calendar_connections', {
  ...
  // Calendar selection
  selectedCalendarIds: json('selected_calendar_ids').$type<string[]>(),
...
export const reminderSettings = pgTable('reminder_settings', {
  ...
  defaultReminders: json('default_reminders').$type<Array<{
    type: string;
  ...
  clientReminderTypes: json('client_reminder_types').$type<string[]>().default(['email', 'sms']),
  clientReminderTiming: json('client_reminder_timing').$type<number[]>().default([1440, 60]), // 24h y 1h antes
