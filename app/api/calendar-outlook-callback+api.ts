...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      console.error('[OutlookCallback] Authorization error:', error);
    ...
    const connectionData = {
      ...
      calendarName: calendarInfo.name || 'Outlook Calendar',
    ...
    console.log('✅ [OutlookCallback] Calendar connected successfully');

    // Redirigir a página de configuración
    return Response.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'https://www.pianoemotion.com'}/settings/calendar-settings?success=outlook_connected`,
      302
    );
  } catch (error) {
    console.error('[OutlookCallback] Error:', error);
    return Response.redirect(
