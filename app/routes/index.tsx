...
export default function RoutesScreen() {
  ...
  const routeGroups = useMemo(() => {
    ...
    clients.forEach(client => {
      const routeName = client.routeGroup || '__unassigned__';
  ...
  const existingRouteNames = useMemo(() => {
    return [...new Set(clients.map(c => c.routeGroup).filter(Boolean))] as string[];
  }, [clients]);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            const clientsInRoute = clients.filter(c => c.routeGroup === routeName);
  ...
  const handleOpenMap = (routeName: string) => {
    const routeClients = clients.filter(c => c.routeGroup === routeName);
              ...
              <View key={client.id} style={[styles.clientItem, { borderColor }]}>
                <View style={styles.clientInfo}>
                  ...
                  <ThemedText style={[styles.clientAddress, { color: textSecondary }]}>
                    {client.city || client.address || 'Sin dirección'}
                  </ThemedText>
                </View>
                {item.name !== 'Sin asignar' && (
                  <Pressable
        ...
        <View style={[styles.statsCard, { backgroundColor: cardBg, borderColor }]}>
          ...
          <View style={styles.statItem}>
            <ThemedText style={[styles.statNumber, { color: success }]}>
              {clients.filter(c => c.routeGroup).length}
            </ThemedText>
