...
async function getHelpSections() {
  try {
...
async function getHelpItems(sectionId?: string) {
  try {
    const connection = await pool.getConnection();
...
async function getHelpSectionsWithItems() {
  try {
    ...
    return (sections as any[]).map((section: any) => ({
      ...section,
      content: (items as any[]).filter((item: any) => item.section_id === section.id),
    }));
  } catch (error) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.error('Error in sections-with-items endpoint:', error);
