...
// Plugin para resolver imports .js que realmente son .ts
const tsResolverPlugin = {
  name: 'ts-resolver',
  setup(build) {
    // Resolver TODOS los imports relativos
    build.onResolve({ filter: /^\.\.?\// }, (args) => {
      let targetPath = path.resolve(args.resolveDir, args.path);
      
      // Si termina en .js, intentar con .ts primero
      ...
      // Si no tiene extensión, intentar resolver con extensiones
      const resolved = resolveWithExtensions(targetPath);
      if (resolved) {
        return { path: resolved };
      }
      
      // Dejar que esbuild maneje el error
      return undefined;
    });
  },
};
...
try {
  await esbuild.build({
    ...
    outdir: 'web-build',
    packages: 'external',
    plugins: [tsResolverPlugin, aliasPlugin],
