...
export function useRecommendations(pianos: Piano[], services: Service[]) {
  ...
  const recommendations = useMemo(() => {
    ...
    pianos.forEach((piano) => {
      ...
      if (piano.alertsEnabled === false) {
        return;
      }

      // Determinar umbrales a usar (personalizados del piano o globales)
      const tuningThresholds = {
        urgent: piano.customThresholdsEnabled && piano.tuningIntervalDays
          ? piano.tuningIntervalDays * 1.5 // 1.5x el intervalo = urgente
          : DEFAULT_THRESHOLDS.tuning.urgent,
        pending: piano.customThresholdsEnabled && piano.tuningIntervalDays
          ? piano.tuningIntervalDays
          : DEFAULT_THRESHOLDS.tuning.pending,
      };

      const regulationThresholds = {
        urgent: piano.customThresholdsEnabled && piano.regulationIntervalDays
          ? piano.regulationIntervalDays * 1.5
          : DEFAULT_THRESHOLDS.regulation.urgent,
        pending: piano.customThresholdsEnabled && piano.regulationIntervalDays
          ? piano.regulationIntervalDays
